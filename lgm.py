...
if __name__ == '__main__':
    ...
    for train_index, test_index in skf.split(all_file, data_label):
        ...
        train_data = lgb.Dataset(pd.DataFrame(train_word_freq), label=train_label)
        num_round = 50
        ...
        predict_label = lgbm.predict(test_word_freq)
        # print(predict_label)
        predicted_label = []
        for num in range(len(predict_label)):
            if predict_label[num] >= 0.5:
                predicted_label.append(1)
            else:
                predicted_label.append(0)

        accuracy = metrics.accuracy_score(test_label, predicted_label)
        precision = metrics.precision_score(test_label, predicted_label)
        recall = metrics.recall_score(test_label, predicted_label)
        f1_score = metrics.f1_score(test_label, predicted_label)
